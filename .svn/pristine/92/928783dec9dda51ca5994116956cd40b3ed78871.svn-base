    ...
    public class UserAccountRepository : IUserAccountRepository
    {
            ...
            using (var db = new eSyaEnterprise())
            {
                ...
                if (lg != null)
                {
                    if (!lg.ActiveStatus)
                    {
                        ...
                        us.Message = "User Id is not activated.please contact system administrator.";
                        return us;
                    }
                    if (lg.BlockSignIn)
                    {
                        us.IsSucceeded = false;
                        us.Message = "User Id is blocked.please contact system administrator.";
                        return us;
                    }
                    ...
                    if (maxUnsuccessfulAttempts > 0)
                    {
                        if (lg.UnsuccessfulLoginAttempts >= maxUnsuccessfulAttempts)
                        {
                            if (unLockLoginAfter > 0)
                            {
                                if (lg.LoginAttemptDate.HasValue)
                                    if (lg.LoginAttemptDate.Value.AddHours((int)unLockLoginAfter) > DateTime.Now)
                                    {
                                        ...
                                        us.IsSucceeded = false;
                                        us.Message = "Your account has been locked out for " + waitingHour.Hours.ToString() +":"+ waitingHour.Minutes.ToString() + " hours due to " + maxUnsuccessfulAttempts.ToString() + " failed login attempts";
                                        return us;
                                    }
                            }
                            else
                            {
                                us.IsSucceeded = false;
                                us.Message = "Your account has been locked due to " + maxUnsuccessfulAttempts.ToString() + " failed login attempts";
                                return us;
                            }
                        }
                    }
                    ...
                    else
                    {
                        ...
                        if ((maxUnsuccessfulAttempts - lg.UnsuccessfulLoginAttempts) > 0)
                            us.Message += "Attempts remaining:" + (maxUnsuccessfulAttempts - lg.UnsuccessfulLoginAttempts).ToString();
            ...
            using (var db = new eSyaEnterprise())
            {
                ...
                if (lg != null)
                {
                    ...
                    if (passwordRepeatationPolicy > 0)
                    {
                        ...
                        var l_ph = await db.GtEuusph.Where(w => w.UserId == userID && w.ActiveStatus)
                        ...
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "You used this password recently. Please choose a different one." };
                        }
                    }
                    //}
        ...
        public async Task<DO_ReturnParameter> ResetUserPassword(int userID, string oldpassword, string newPassword, int passwordRepeatationPolicy)
        {
            using (var db = new eSyaEnterprise())
            {
                ...
                {
                    return new DO_ReturnParameter { Status = false, Message = "Invalid Current Password" }; ;
                }
            }
        }
        ...
        public async Task<List<DO_MainMenu>> GetUserMenulist_O(int businessKey, int userID)
        {
            try
            {
                using (eSyaEnterprise db = new eSyaEnterprise())
                {
                    ...
                    List<DO_MainMenu> l_MenuList = new List<DO_MainMenu>();
                    var mainMenus = db.GtEcmamn.Where(w => userForms.Any(m => m.MainMenuId == w.MainMenuId)
        ...
        public async Task<List<DO_MainMenu>> GetUserMenulist(int businessKey, int userID)
        {
            try
            {
                using (eSyaEnterprise db = new eSyaEnterprise())
                {
                    var us = db.GtEuusrl.Where(w => w.BusinessKey == businessKey && w.UserId == userID && w.ActiveStatus).FirstOrDefault();
        ...
        public async Task<DO_UserFormRole> GetFormActionByUser(int businessKey, int userID, string navigationURL)
        {
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    ...
                    //{
                    var us = db.GtEuusrl.Where(w => w.BusinessKey == businessKey && w.UserId == userID && w.ActiveStatus).FirstOrDefault();
                     var uf = db.GtEuusac.Where(w => w.UserGroup == us.UserGroup
        ...
        public async Task<List<DO_MainMenu>> GeteSyaMenulist()
        {
            try
            {
                ...
                {
                    List<DO_MainMenu> l_MenuList = new List<DO_MainMenu>();
                    var mainMenus = db.GtEcmamn.Where(w => w.ActiveStatus == true).OrderBy(o => o.MenuIndex);
                    foreach (var m in mainMenus)
                    {
                        DO_MainMenu do_MainMenu = new DO_MainMenu();
                        do_MainMenu.MainMenuId = m.MainMenuId;
